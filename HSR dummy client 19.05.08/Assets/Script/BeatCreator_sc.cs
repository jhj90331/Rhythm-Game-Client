...
namespace DummyClient
{
	...
	public class BeatCreator_sc : MonoBehaviour
	{
		...
		// 플레이어의 노트들
		public List<NoteObj_sc> noteObj_Line_1;		// 1키 노트들
		public List<NoteObj_sc> noteObj_Line_2;		// 2키 노트들
		public List<NoteObj_sc> noteObj_Line_3;		// 3키 노트들
		public List<NoteObj_sc> noteObj_Line_4;		// 4키 노트들
		public List<NoteObj_sc> noteObj_Line_5;		// 5키 노트들
		public List<NoteObj_sc> bar_Line;			// 중간 마디들
		...
		// 적 플레이어의 노트들
		public List<NoteObj_sc> enemy_noteObj_Line_1;     // 1키 노트들
		public List<NoteObj_sc> enemy_noteObj_Line_2;     // 2키 노트들
		public List<NoteObj_sc> enemy_noteObj_Line_3;     // 3키 노트들
		public List<NoteObj_sc> enemy_noteObj_Line_4;     // 4키 노트들
		public List<NoteObj_sc> enemy_noteObj_Line_5;     // 5키 노트들
		public List<NoteObj_sc> enemy_bar_Line;           // 중간 마디들
		...
		IEnumerator Create()
		{
			...
			{
				bgmPlayer.Play();
				isBgmPlay = true;	// BGM이 재생되고 있지 않으면 false 재생되고 있으면 true
									// 이어폰 빼면 false되버림
			}
			...
			if (Time.time >= nextTime && isBgmPlay == false)
			{
				if (noteObj_Line_1.Count > noteIndex_1)
				{
					if (nextTime >= (noteObj_Line_1[noteIndex_1].noteTime))
					{
						...
						enemy_noteObj_Line_1[noteIndex_1].isStart = true;
						noteIndex_1++;
					}
				}
				if (noteObj_Line_2.Count > noteIndex_2)
				{
					if (nextTime >= (noteObj_Line_2[noteIndex_2].noteTime))
					{
						...
						enemy_noteObj_Line_2[noteIndex_2].isStart = true;
						noteIndex_2++;
					}
				}
				if (noteObj_Line_3.Count > noteIndex_3)
				{
					if (nextTime >= (noteObj_Line_3[noteIndex_3].noteTime))
					{
						...
						enemy_noteObj_Line_3[noteIndex_3].isStart = true;
						noteIndex_3++;
					}
				}
				if (noteObj_Line_4.Count > noteIndex_4)
				{
					if (nextTime >= (noteObj_Line_4[noteIndex_4].noteTime))
					{
						...
						enemy_noteObj_Line_4[noteIndex_4].isStart = true;
						noteIndex_4++;
					}
				}
				if (noteObj_Line_5.Count > noteIndex_5)
				{
					if (nextTime >= (noteObj_Line_5[noteIndex_5].noteTime))
					{
						...
						enemy_noteObj_Line_5[noteIndex_5].isStart = true;
						noteIndex_5++;
					}
				}
				if (bar_Line.Count > barIndex)
				{
					if (nextTime >= bar_Line[barIndex].noteTime)
					{
						...
						enemy_bar_Line[barIndex].isStart = true;
						barIndex++;
					}
				}
