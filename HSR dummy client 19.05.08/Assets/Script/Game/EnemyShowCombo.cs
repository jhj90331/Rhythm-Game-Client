...
namespace DummyClient
{
	...
	public class EnemyShowCombo : MonoBehaviour
	{
		...
		void Update()
		{
			if (gameManager.enemy_playerCombo != 0)
			{
				if (gameManager.enemy_playerCombo != enemy_prevCombo)
				{
					intToSpr(gameManager.enemy_playerCombo, ref showSpr);
					...
					for (int i = 0; i < count; ++i)
					{
						...
						// 콤보수 위치조정 할 것!
						tr.position = new Vector3(((float)i * 1.7f) + 20.0f, transform.position.y, transform.position.z);
						if (count == 2)
						{
					...
					enemy_prevCombo = gameManager.playerCombo;
				}
			}
			else
			{
